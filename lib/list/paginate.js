...
function paginate (options, callback) {
	...
	var countQuery = model.countDocuments(options.filters);
	...
	query.exec = function (cb) {
		return countQuery.exec()
			.then(function (count) {
				...
				return query._original_exec().then(function (results) {
	...
	if (callback) {
		query.exec().then(function(result) { callback(null, result); }).catch(callback);
		return;
	} else {
		return query;
	}
