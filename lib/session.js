...
function hash(str) {
	...
	// hash using sha256var crypto = require('crypto');
	var keystone = require('../');
	var scmp = require('scmp');
	var utils = require('keystone-utils');
	var _ = require('lodash');
	...
	var doSignin = function (lookup, req, res, onSuccess, onFail) {
		...
		if (typeof lookup.email === 'string' && typeof lookup.password === 'string') {
			...
			User.model.findOne({ email: emailRegExp }).exec()
				.then(user => {
					if (user) {
			...
			User.model.findById(userId).exec()
				.then(user => {
	...
	exports.persist = async function (req, res, next) {
		...
		if (keystone.get('cookie signin') && !req.session.userId && req.signedCookies['keystone.uid'] && req.signedCookies['keystone.uid'].indexOf(':') > 0) {
			...
			try {
				const user = await User.model.findById(req.session.userId).exec();
				req.user = user;
				next();
			} catch (err) {
				return next(err);
