...
Content.prototype.fetch = function (page, callback) {
	...
	if (page) {
		...
		this.AppContent.findOne({ key: page }).exec().then(function(result) {
		...
		this.AppContent.find().exec().then(function(results) {
			var data = {};
...
Content.prototype.store = function (page, content, callback) {
	...
	this.AppContent.findOne({ key: page }).exec().then(function(doc) {
		if (doc) {
			if (doc.content) {
		...
		doc.save().then(() => callback()).catch(callback);
	}).catch(function(err) {
		return callback(err);
	});
